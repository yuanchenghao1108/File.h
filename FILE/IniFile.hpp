...
private:
	...
	// --------------------------------------------------------
	// 读取config文件  
	INI_RES __fastcall _get_key(const char* mAttr, const char* cAttr, char* value);
	string __fastcall _trim(const char* source);
	// --------------------------------------------------------
protected:
public:
	// --------------------------------------------------------
	int __fastcall ReadInt(const char* mAttr,const char* cAttr,int def_value = 0);
	char * __fastcall ReadString(const char* mAttr, const char* cAttr, string def_value = "NULL");
	// --------------------------------------------------------
